        ...
        if(uploaded_file!=None):
            with pdfplumber.open(uploaded_file) as pdf:
                ...
                for page,context in enumerate(pdf.pages):
                    text=text+context.extract_text()
...
if(selected=='URL'):
    ...
    if(st.button('Analyze T&C')):
        if(link):
            ...
            url_text=soup.find_all(string=True)
            
            remove_list=[
                '[document]',  # Document type declaration
                'noscript',    # Elements rendered when JavaScript is not supported
                'header',      # Website header section
                'footer',      # Website footer section
                'html',        # Root HTML tag
                'meta',        # Metadata tags (like charset, viewport, etc.)
                'head',        # Head section (metadata, scripts, etc.)
                'input',       # Input fields (e.g., forms, search bars)
                'script',      # JavaScript code
                'style',       # CSS styles
                'iframe',      # Inline frames (usually embeds)
                'canvas',      # Used for rendering graphics
                'svg',         # Vector graphics
                'link',        # Links to stylesheets or other external resources
                'form',        # Form elements
                'button',      # Buttons
                'aside',       # Sidebar content
                'nav',         # Navigation menus
                'ins',         # Advertisements or promotional content
                'iframe',      # Embedded media like videos or maps
                'embed',       # Embeds for external content
